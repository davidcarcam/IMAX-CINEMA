...
namespace EXPO60.Modelo
{
    class Funciones_proveedores
    {
        public static int insertarProveedor(Constructor_Proveedores add)
        {
            ...
            try
            {
                MySqlCommand cmdadd = new MySqlCommand(string.Format("INSERT INTO proveedores(nombre, apellido, correo, direccion, dui, telefono  ) VALUES ('{0}','{1}','{2}','{3}','{4}','{5}')", add.nombre, add.apellido, add.correo,add.direccion, add.DUI, add.telefono), Conexion.ObtenerConexion());
        ...
        public static bool actualizarProveedor(Constructor_Proveedores upd)
        {
            ...
            try
            {
                MySqlCommand cmdupd = new MySqlCommand(string.Format("UPDATE proveedores SET nombre = '{0}', apellido = '{1}', correo = '{2}', direccion = '{3}', dui = '{4}', telefono = '{5}'", upd.nombre, upd.apellido, upd.correo,upd.direccion, upd.DUI, upd.telefono), Conexion.ObtenerConexion());
