...
namespace EXPO60.Modelo
{
    class Funciones_Asientos
    {
        ...
        public static DataTable Mostrar_Asientos()
        {
            ...
            try
            {
                string query3 = "SELECT id_asiento AS ID,fila,num_asiento,estado_as,num_sala FROM asientos INNER JOIN fila ON asientos.id_fila = fila.id_fila INNER JOIN numeracion ON asientos.id_num = numeracion.id_num INNER JOIN estado_asiento ON estado_asiento.id_estado_asiento = asientos.id_estado_asiento INNER JOIN salas ON asientos.id_sala = salas.id_sala";
